...
class Achievement extends Equatable {
  ...
  final String description; // How to earn it (e.g., "Run a total distance of 100 kilometers")
  ...
  @override
  List<Object?> get props => [id, name, description, icon, type, threshold, dateEarned, workoutId];
   ...
   Map<String, dynamic> toMap() {
    ...
    factory Achievement.fromMap(Map<String, dynamic> map) {
       return Achievement(
         ...
         type: AchievementType.values.firstWhere((e) => e.name == map['type'], orElse: () => AchievementType.specialEvent),
         threshold: (map['threshold'] as num?)?.toDouble() ?? 0.0,
         dateEarned: map['dateEarned'] != null ? DateTime.tryParse(map['dateEarned'] as String) : null,
...
final List<Achievement> predefinedAchievements = [
   const Achievement(id: 'total_dist_10k', name: '10 Kilometers', description: 'Run a total of 10 kilometers.', icon: Icons.directions_run, type: AchievementType.distanceTotal, threshold: 10000),
   const Achievement(id: 'total_dist_100k', name: '100 km Club', description: 'Run a total distance of 100 kilometers.', icon: Icons.emoji_events_outlined, type: AchievementType.distanceTotal, threshold: 100000),
    const Achievement(id: 'single_dist_10k', name: 'First 10k', description: 'Complete a single run of 10 kilometers or more.', icon: Icons.looks_one, type: AchievementType.distanceSingle, threshold: 10000),
    const Achievement(id: 'single_dist_hm', name: 'Half Marathoner', description: 'Complete a single run of 21.1 kilometers or more.', icon: Icons.military_tech_outlined, type: AchievementType.distanceSingle, threshold: 21097.5),
    const Achievement(id: 'count_workouts_10', name: 'Getting Started', description: 'Complete 10 workouts.', icon: Icons.fitness_center, type: AchievementType.countWorkouts, threshold: 10),
