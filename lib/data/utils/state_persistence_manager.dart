...
abstract class StatePersistenceManager {
  ...
  Future<void> saveWorkoutState(Workout? workout, WorkoutTrackingState trackingState);

  /// Loads the previously saved workout state, if any.
  Future<({Workout? workout, WorkoutTrackingState? trackingState})?> loadWorkoutState();
...
class SharedPreferencesStatePersistenceManager implements StatePersistenceManager {
  static const _workoutStateKey = 'active_workout_state_v3'; // Use versioned key

  @override
  Future<void> saveWorkoutState(Workout? workout, WorkoutTrackingState trackingState) async {
    // Only save if workout is in a resumable state
    if (workout != null && (trackingState == WorkoutTrackingState.active || trackingState == WorkoutTrackingState.paused)) {
      try {
        ...
        Log.d("Saved workout state (State: ${trackingState.name}) to SharedPreferences.");
      } catch (e, s) {
        Log.e("Error saving workout state to SharedPreferences", error: e, stackTrace: s);
       ...
       Log.d("Clearing workout state because current state is not active/paused.");
       await clearWorkoutState();
    }
  }

  @override
  Future<({Workout? workout, WorkoutTrackingState? trackingState})?> loadWorkoutState() async {
    ...
    try {
      ...
      if (stateString != null) {
        ...
        final String? trackingStateName = loadedState['trackingStateName'] as String?;

        if (workoutJsonString != null && trackingStateName != null) {
          // Decode workout JSON string back to map, then create Workout object
          final workoutMap = jsonDecode(workoutJsonString) as Map<String, dynamic>;
          ...
          final trackingState = WorkoutTrackingState.values.firstWhere(
             (e) => e.name == trackingStateName, orElse: () => WorkoutTrackingState.idle);

           // Validate loaded state - only return if it's active or paused
           if (trackingState == WorkoutTrackingState.active || trackingState == WorkoutTrackingState.paused) {
              Log.i("Loaded resumable workout state (State: ${trackingState.name}) from SharedPreferences.");
              return (workout: workout, trackingState: trackingState);
           } else {
              Log.w("Loaded state is not resumable ($trackingStateName). Clearing.");
              await clearWorkoutState(); // Clear non-resumable state
              return null;
           }
        } else {
           Log.w("Loaded state string is missing workout or trackingStateName. Clearing.");
      ...
      Log.e("Error loading workout state from SharedPreferences", error: e, stackTrace: s);
  ...
  Future<void> clearWorkoutState() async {
    try {
      ...
      Log.e("Error clearing workout state from SharedPreferences", error: e, stackTrace: s);
    }
  }
}